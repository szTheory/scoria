defmodule Scoria.Observe.ReviewerBroadcast do
  ...
  def span_delta(_metadata) do
    Logger.debug("ReviewerBroadcast.span_delta/1 dropped: missing tenant_id")
    :dropped
  end
  ...
  def hitl_request(_tenant_id, _projection_map) do
    Logger.debug("ReviewerBroadcast.hitl_request/2 dropped: missing tenant_id")
    :dropped
  end
  ...
  def approval_decided(_tenant_id, _approval_id, _status) do
    Logger.debug("ReviewerBroadcast.approval_decided/3 dropped: missing tenant_id")
    :dropped
  end
