defmodule Scoria.SRE.Adapters.Chimeway do
  @moduledoc """
  Optional Chimeway-facing alert adapter.

  Review and page envelopes are shaped locally and only dispatched when the host
  application opts in with a configured transport.
  """
  ...
  def publish(envelope) when is_map(envelope) do
    envelope
    |> to_alert_envelope(:chimeway)
    |> dispatch(:sre_chimeway_dispatcher, :chimeway)
  end
