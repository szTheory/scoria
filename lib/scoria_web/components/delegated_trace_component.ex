defmodule ScoriaWeb.DelegatedTraceComponent do
  @moduledoc """
  Reviewer-facing delegated run trace component.
  """
  ...
  def render(assigns) do
    ...
    <.notebook
      id="delegated-trace"
      title="Delegated Trace"
      eyebrow="Delegated Trace"
      selected_tab="delegated"
      empty={@delegated_handoffs == []}
    >
      <:empty_slot>
        <.evidence_empty title="No Delegated Traces Recorded">
      ...
      <:tab key="delegated" label="Delegated">
        <div class="space-y-4">
          <.evidence_section
            ...
            description="Review bounded delegated trace lineage from the curated runtime detail instead of reconstructing it from raw workflow rows."
          >
            <:actions>
              <a href="#delegated-trace" class="scoria-button scoria-button--ghost scoria-button--sm">
                Inspect Delegated Trace
              </a>
            </:actions>
  ...
  defp handoff_section(assigns) do
    ...
    >
      ...
      <div class="mt-3 grid gap-3 lg:grid-cols-2">
        <.evidence_section title="Scoped Context Preview">
          <%= if preview_context(@delegated) == [] do %>
            <.evidence_empty title="No scoped context recorded yet.">
              No scoped context recorded yet.
            </.evidence_empty>
          <% else %>
      ...
      <.raw_evidence label="View full scoped context">
    handoff input
