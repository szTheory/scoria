defmodule ScoriaWeb.SemanticCacheTraceNotebookComponent do
  @moduledoc """
  Reviewer-facing semantic cache trace notebook component.
  """
  ...
  def render(assigns) do
    ...
    <.notebook
      ...
      id="semantic-cache-trace-notebook"
      title="Semantic cache trace inspection"
      eyebrow="semantic cache trace notebook"
      selected_tab="semantic"
    >
      <:tab key="semantic" label="Semantic">
        <div class="space-y-4">
          <.evidence_section
            title="Semantic cache trace groups"
            description="Cache execution details keep semantic cache verdict, compatibility, provenance, lifecycle, and append-only events on one page."
          >
  ...
  defp group_section(assigns) do
    ...
    <.evidence_section
      title={@title}
      description="Curated semantic cache trace details from runtime metadata and durable cache truth."
  ...
  defp events_section(assigns) do
    ...
    <.evidence_section
      ...
      description="Semantic cache entry events stay inspectable instead of collapsing into a generic miss."
  ...
  defp render_value("semantic_reuse"), do: "Semantic cache trace reused a cached answer."

  defp render_value("live_execution_admitted"),
    do: "Normal runtime path executed and admitted fresh semantic cache trace details."

  defp render_value("live_execution_writeback_rejected"),
    do: "Normal runtime path executed and writeback_rejected semantic cache trace details."
