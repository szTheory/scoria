defmodule Scoria.Workflows.ApprovalWriteInvariantGuardTest do
  ...
  #   1. workflows.ex:436 — the creation-time second `Approval.changeset |> update!`
  ...
  #   2. workflows.ex:888 — the single decision write inside `approve/3` that
  ...
  # them in workflows.ex (most recently by RAIL-01's `halt_run/3`). Re-verify
  # with `grep -n "Approval.changeset" lib/scoria/workflows.ex` after any edit
  # that adds/removes lines above these two call sites.
  @allowed_approval_updates MapSet.new([
                              {"lib/scoria/workflows.ex", 436},
                              {"lib/scoria/workflows.ex", 888}
                            ])
