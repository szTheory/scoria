...
defmodule ScoriaWeb.ApprovalsLiveIntegrationTest do
  ...
  test "focused approval surfaces blocking modal without send/2" do
    ...
    eventually(fn -> render(view) =~ "publish approval" end)
  ...
  test "approve decision clears modal via producer path" do
    ...
    eventually(fn ->
      html = render(view)
      html =~ "publish approval" and html =~ "publish" and not (html =~ "super-secret-key")
    end)
    ...
    eventually(fn ->
      ...
      not (html =~ "publish approval") and Repo.get!(Approval, approval_id).status == "approved" and
  ...
  test "dismiss closes modal without approving via producer path" do
    ...
    eventually(fn ->
      html = render(view)
      html =~ "publish approval" and html =~ "publish"
    end)
    ...
    eventually(fn ->
      ...
      not (html =~ "publish approval") and html =~ "publish" and
  ...
  test "stale approval decision surfaces friendly flash via producer path" do
    ...
    eventually(fn -> not (render(view) =~ "publish approval") end)
  ...
  test "non-focused approval highlights inbox without replacing modal via producer path" do
    ...
    eventually(fn ->
      ...
      html =~ "publish approval" and html =~ "publish" and html =~ "other_tool" and
  ...
  test "reject decision clears modal and keeps run paused" do
    ...
    eventually(fn ->
      ...
      not (html =~ "publish approval") and Repo.get!(Approval, approval_id).status == "rejected" and
  ...
  test "plain inbox highlights producer approval without auto-opening drawer" do
    ...
    eventually(fn ->
      ...
      html =~ "publish" and html =~ ~s(data-highlight="true") and
        not (html =~ "publish approval") and not (html =~ "super-secret-key")
    end)
  end
