...
**Claim precision (revised 2026-07-29, phase 58 discuss, 58-CONTEXT.md D-02/D-39).** As shipped through Phase 57, the gate escalates when a tool **declares** all three legs — a self-declaration gate over host-authored classification metadata, not an observation gate. Scanner-derived taint does not reach the gate: `confluence_input/2` builds leg witnesses solely from the tool's declared `%Classification{}` and `leg_witness(true)` hardcodes `%{source: :declared}` (`executor.ex:705-731`), with `executor.ex:737-739` recording that `:default_tier` and `:scanner_infra` are not constructible through any live call path. Phase 57's D-13 promised the observed-scanner half and it was never wired; Phase 58.1 / HOOK-03 closes it, at which point this note is upgraded. Declared-classification confluence enforcement with audited, replayable escalation is real and rare — but it must be claimed at that precision, and `guides/security-boundary.md` (BOUND-01) states the same limitation adopter-side.
...
**Current posture:**
- **v3.7 Portcullis is in flight — 4 of 5 phases complete.** Phase 55 (Content Trust & Taint Substrate) shipped the missing untrusted-content leg: trust tiers on knowledge chunks/tool outputs, prompt-assembly spotlighting, and the BYO `scan/2` hook. Phase 56 (Tool-Declared Trifecta Classification, verified 4/4, 2026-07-28) closed the `approval_sensitive: false` footgun: a tool declares its trifecta legs plus an `action_class` once via `Scoria.MCP.Tool`, `MCP.Executor` resolves that declaration exactly once per call before `replay_gate/3`, unclassified tools fail closed to an inspectable default with `[:scoria, :class, :unclassified]` telemetry plus an opt-in `require_tool_classification` hard refusal, the resolution is persisted to `step.result_envelope` and projected as `scoria.classification.*`, and **all five** fail-open seams consume it — including the two the requirement text originally missed (`Connectors.Invocation.build_seam/2`, which decides replay before the executor is reached, and `Workflows.Runtime`'s `%{local_classification: :pure}` step-granularity bypass). Phase 56.1 (Per-Run Rails, split out of 56, verified 4/4, 2026-07-28) closed RAIL-01: all three rails enforce with an audited, non-resurrectable halt. **Phase 57 (Confluence Escalation Gate, verified 5/5, 2026-07-29) shipped the milestone's flagship differentiator** — when private data, untrusted content and an exfil-capable action co-occur in one tainted path, `MCP.Executor` decides and refuses *before* the tool's execution task starts, pausing the run's **step** (not the whole run) through `Workflows.mark_waiting_for_approval/3` so the escalation stays resumable. Enforcement is evidence-graded: the `declared` grade escalates from the shipped default while the three ungated grades (`unclassified`/`scanner_infra`/`default_tier`) emit telemetry only, so an adopter who has declared nothing is never silently bricked (opt-in strict mode extends enforcement to all four). Every escalation is audited to the outbox and replayable, and the reviewer drawer renders the named combination, each lit leg with its witness source, and the evidence grade — read from that escalation's own frozen audit row via `blocker_audit_outbox_event_id`, scoped by `event_type` **and** `workflow_run_id` so a dangling or cross-run pointer yields no row rather than fabricated confidence, batched once per page. All four GATE requirements complete. Remaining: 58 safety hooks/boundary/Govern surface.
...
- **Phase 56.1 complete (2026-07-28, v3.7 Portcullis):** Per-run rails (RAIL-01) — a single run can no longer exceed its own step/call/time budget unnoticed. Seven `rail_*` columns on `ai_workflow_runs` (the phase's one migration); `Scoria.Workflows.Rails.admit_step/2` and `admit_tool_call/2` are atomic CAS admissions (POST-increment semantics proven by a committed spike); `halt_run/3` writes exactly one audited `run.rail.tripped` outbox row inside the halt transaction and six terminality guards (G1–G6) close every claim/retry/resume/clamp resurrection path. `Scoria.Runtime.Rails.resolve/1` is a two-rung ladder where a per-run value wins outright in both directions — deliberately not tighten-only — and malformed rail config refuses the *run*, never the boot. `max_tool_calls` is enforced as the first gate in `MCP.Executor.execute/4` and at `Connectors.Invocation.invoke/4`, so `:historical_stub`/`:blocked` short-circuits still consume budget. `max_active_ms` measures active time, never wall clock: `rail_paused_at`/`rail_paused_ms` are uncastable and derived solely by `derive_rail_pause_accounting/1` inside `changeset/2`, structurally closing the sibling-completion accumulator leak; check order is fixed at `max_active_ms → max_steps → max_tool_calls`. Rails default to unlimited with counting always on, so limits can be sized from real traffic; the no-run-attribution surface (inbound JSON-RPC via `MCP.Router`) is an explicit telemetried no-op (`[:scoria, :run, :rail, :skipped]`), not a silent gap. Three `scoria.rail.*` Semconv keys ride the existing denied-step span (no second span), and `[:scoria, :run, :rail, :observed]` fires once per run at terminal transition with steps/tool_calls/active_ms/paused_ms/wall_ms kept as distinct measurements. Adopter guide `guides/capabilities/per-run-rails.md` registered across all four docs/package contracts, with the migration disclosed in CHANGELOG alongside (not replacing) the pinned terminology-migration sentence. Verified passed — 4/4 success criteria, 34/34 must-haves checked against source; 6/6 plans, 27 commits. v3.7 Portcullis 3/5 phases complete.
...
---
*Last updated: 2026-07-29 after Phase 57 (Confluence Escalation Gate) completed and verified 5/5 — GATE-01..04 all closed, the flagship confluence gate escalates before execution with graded, audited, replayable evidence rendered to the reviewer. v3.7 Portcullis 4/5 phases complete.*
