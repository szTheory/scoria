...
**Current posture:**
- **v3.7 Portcullis is in flight — 2 of 5 phases complete.** Phase 55 (Content Trust & Taint Substrate) shipped the missing untrusted-content leg: trust tiers on knowledge chunks/tool outputs, prompt-assembly spotlighting, and the BYO `scan/2` hook. Phase 56 (Tool-Declared Trifecta Classification, verified 4/4, 2026-07-28) closed the `approval_sensitive: false` footgun: a tool declares its trifecta legs plus an `action_class` once via `Scoria.MCP.Tool`, `MCP.Executor` resolves that declaration exactly once per call before `replay_gate/3`, unclassified tools fail closed to an inspectable default with `[:scoria, :class, :unclassified]` telemetry plus an opt-in `require_tool_classification` hard refusal, the resolution is persisted to `step.result_envelope` and projected as `scoria.classification.*`, and **all five** fail-open seams consume it — including the two the requirement text originally missed (`Connectors.Invocation.build_seam/2`, which decides replay before the executor is reached, and `Workflows.Runtime`'s `%{local_classification: :pure}` step-granularity bypass). Remaining: 56.1 per-run rails, 57 confluence escalation gate, 58 safety hooks/boundary/Govern surface.
...
---
*Last updated: 2026-07-28 after Phase 56.1 (Per-Run Rails) completed and verified 4/4 — RAIL-01 closed, all three rails enforce with an audited non-resurrectable halt. v3.7 Portcullis 3/5 phases complete.*
