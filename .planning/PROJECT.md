...
- Phase 42 is complete and verified: eval now fails closed across real subject-output capture, ExactMatch scoring, `not_scored` verdict semantics, online negative-signal scoring, and ReleaseGate verdict consultation (31/31 must-haves).
- **The next Hex release (`0.1.3`, PR #12) is still HELD** behind the remaining SEED-006 (999.1) fixes — knowledge cross-tenant retrieval leak, dashboard auth bypass, and final correctness sweep must be fixed before publishing over shipped `0.1.2`.
...
**Target features:**
- **Eval fails CLOSED** — execute the real subject prompt (kill the `expected_output["answer"]` shortcut), a real deterministic scorer, and `:not_scored` + `ReleaseGate` consulting `threshold_verdict` so no run is ever fake-green (EVAL-01..05). **Validated in Phase 42** (2026-07-05).
...
- **Phase 42 complete (2026-07-05, v3.4 Pre-1.0 Trust & Security Hardening):** Eval fails closed (EVAL-01..05) — subject output is captured from `Workflows.Step.result_envelope["output"]`, offline replay and judge paths resolve that frozen capture through `SubjectOutput`, `ExactMatch` provides the first deterministic actual-vs-expected scorer, missing/unknown/unmeasured items persist `not_scored` and verdict `inconclusive`, online deterministic scoring emits only negative evidence, and `Runtime.ReleaseGate` consults latest completed eval verdicts before release. Verified passed (31/31 must-haves); focused phase gate passed `make build` plus 63 tests.
...
- [x] **EVAL-01..05**: Eval fails CLOSED — real subject output, ≥1 real deterministic scorer, `:not_scored`, `ReleaseGate` consults `threshold_verdict`, online scoring stops fabricating from `sample_reason`. Validated in Phase 42 (31/31 must-haves).
...
---
*Last updated: 2026-07-05 after completing Phase 42 eval fails-closed.*
