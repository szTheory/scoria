...
- [ ] **EVAL-02**: Captured screenshots can be critiqued against a 9-dimension rubric (brand-fit, consistency, hierarchy, affordance/least-surprise, accessibility, responsive, motion, microcopy, density) to produce structured per-screen findings.
- [ ] **EVAL-03**: The screenshot + critique harness ships as committed dev-only tooling (not merge-blocking CI) with documented usage.
- [ ] **EVAL-04**: `dev_seed.exs` populates every dashboard screen — Reviews, Incidents, Eval Workbench, and Prompt Registry included — so each renders at its most useful when clicked through.
