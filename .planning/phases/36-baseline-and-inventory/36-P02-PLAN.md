...
<task type="auto">
  ...
  <action>
    ...
    Every row must include all required keys from D-06, use stable IDs from D-03, keep canonical fields in JSON per D-04, use JTBD-first wording per D-25, include brand/design-pillar considerations per D-26 and D-27, and attach `risk_refs` only to IDs present in JSON `risks` per D-18 through D-23. Do not edit any read-only input file.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    Rows cover current foundations, `ScoriaWeb.UI` primitives, component groups, LiveView pages, CSS/JS hooks, fixtures, tests, docs, and one-off patterns.
