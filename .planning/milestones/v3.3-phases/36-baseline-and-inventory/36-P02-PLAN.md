...
<task type="auto">
  ...
  <action>
    ...
    Add rows for foundations: brandbook tokens and docs, CSS reset/tokens/base/components/motion/utilities, and DS-06 drift guard evidence. Add primitive rows for `ScoriaWeb.UI` function component families including badges, buttons, icon buttons, panels/page sections, metrics/overview stats/signal summaries, IDs/timestamps/metadata rows, raw evidence/code, drawers/modals, toasts/flash, forms, tables/lists, empty/skeleton states, command palette shell, notebooks, and evidence actions. Add component-group rows from `lib/scoria_web/components/`, layout shell rows for app/root layouts, page rows for LiveViews under `lib/scoria_web/live/`, hook rows for named capabilities in `assets/js/scoria.js`, fixture rows for fixture/proof inputs under `test/fixtures`, relevant fixture-support files under `test/support`, recursive `priv/shots` proof artifacts, and `priv/dev` fixture/proof inputs; add test rows for ExUnit and Playwright proof surfaces, doc rows for maintainer/UAT/brand/proof docs, and one-off rows where page-local markup or proof gaps imply noncanonical patterns.

    Treat fixture/proof source reconciliation as mandatory. Recursively discover existing files under `test/fixtures`, `test/support`, `priv/shots`, and `priv/dev`; exclude only intentionally generated, vendor, binary, or report-heavy paths such as `node_modules`, Playwright report directories, `test-results`, screenshot/binary image files that are summarized elsewhere, `.DS_Store`, `_build`, `deps`, `dist`, `build`, `coverage`, and temporary cache outputs. Every discovered fixture/proof input must be represented by a row whose `layer` is `fixture`, `test`, or `doc` as appropriate and whose `owner_path` or `evidence` matches the repository-relative source path, or by a `documented_exclusions` entry with `source`, `reason`, and `reviewed_by_phase`.
  ...
  <verify>
    ...
    <automated>node -e "const fs=require('fs'), path=require('path'), cp=require('child_process'); const j=JSON.parse(fs.readFileSync('.planning/phases/36-baseline-and-inventory/36-inventory.json','utf8')); const rows=j.rows||[]; const documented=j.documented_exclusions||[]; const exclusions=new Set(documented.map(e=>e.source)); for (const e of documented) for (const k of ['source','reason','reviewed_by_phase']) if (!(k in e) || !String(e[k]).trim()) throw new Error('bad exclusion missing '+k); const has=(source, layer)=>rows.some(r=>r.layer===layer && (r.owner_path===source || (r.evidence||[]).some(ev=>ev===source || ev.startsWith(source+':') || ev.startsWith(source+'#')))); const hasAny=(source, layers)=>rows.some(r=>layers.includes(r.layer) && (r.owner_path===source || (r.evidence||[]).some(ev=>ev===source || ev.startsWith(source+':') || ev.startsWith(source+'#')))); const list=(cmd)=>cp.execSync(cmd,{encoding:'utf8',shell:'/bin/bash'}).split('\\n').map(s=>s.trim()).filter(Boolean); const requirePath=(source, layer)=>{ if (!has(source, layer) && !exclusions.has(source)) throw new Error('inventory missing '+layer+' source '+source); }; for (const p of list('find assets/css -maxdepth 1 -type f -name \"*.css\" | sort')) requirePath(p,'foundation'); for (const p of list('find lib/scoria_web/live -type f \\( -name \"*.ex\" -o -name \"*.heex\" \\) | sort')) requirePath(p,'page'); for (const p of list('find lib/scoria_web/components -type f \\( -name \"*.ex\" -o -name \"*.heex\" \\) | sort')) requirePath(p, p.includes('/layouts/') ? 'page' : 'component-group'); for (const p of list('find test/scoria_web -type f -name \"*_test.exs\" | sort')) requirePath(p,'test'); for (const p of list('find priv/dev/e2e -type f -name \"*.mjs\" | sort')) requirePath(p,'test'); for (const p of list('find brandbook docs -type f \\( -name \"*.md\" -o -name \"*.json\" -o -name \"*.css\" \\) | sort')) requirePath(p,'doc'); const skipDirs=new Set(['node_modules','playwright-report','test-results','screenshots','screenshot','_build','deps','dist','build','coverage','.cache','tmp']); const skipExt=new Set(['.png','.jpg','.jpeg','.gif','.webp','.avif','.heic','.ico','.pdf','.zip','.gz','.tgz','.mp4','.mov','.webm']); const proofRoots=['test/fixtures','test/support','priv/shots','priv/dev']; const discovered=[]; const walk=(p)=>{ if (!fs.existsSync(p)) return; const st=fs.statSync(p); const parts=p.split(path.sep); if (parts.some(part=>skipDirs.has(part)) || path.basename(p)==='.DS_Store') return; if (st.isDirectory()) { for (const child of fs.readdirSync(p).map(name=>path.join(p,name)).sort()) walk(child); return; } if (!st.isFile() || skipExt.has(path.extname(p).toLowerCase())) return; discovered.push(p); }; proofRoots.forEach(walk); for (const p of discovered.sort()) if (!hasAny(p,['fixture','test','doc']) && !exclusions.has(p)) throw new Error('inventory missing fixture/proof source '+p); const ui=fs.readFileSync('lib/scoria_web/ui.ex','utf8'); for (const name of [...ui.matchAll(/^\\s*def\\s+([a-zA-Z0-9_!?]+)\\s*\\(/gm)].map(m=>m[1]).filter(n=>!n.startsWith('__')).sort()) if (!rows.some(r=>r.layer==='primitive' && (r.replacement_or_owner||'').includes('ScoriaWeb.UI.'+name+'/'))) throw new Error('missing primitive ScoriaWeb.UI.'+name); const js=fs.readFileSync('assets/js/scoria.js','utf8'); for (const name of [...js.matchAll(/Hooks\\.([A-Za-z0-9_]+)\\s*=/g)].map(m=>m[1]).sort()) if (!rows.some(r=>r.layer==='hook' && ((r.name||'').includes(name) || (r.replacement_or_owner||'').includes(name)))) throw new Error('missing hook '+name);"</automated>
  ...
  <acceptance_criteria>
    `36-inventory.json` has rows or documented exclusions for every discovered source-scan item: `ScoriaWeb.UI` component defs, `lib/scoria_web/live/**`, named hooks in `assets/js/scoria.js`, CSS files, ExUnit tests, Playwright specs, docs/brandbook paths, recursive fixture/proof inputs under `test/fixtures`, `test/support`, `priv/shots`, and `priv/dev`, and one-off/missing documented exclusions.
    ...
    Source-scan reconciliation fails on omitted current foundations, `ScoriaWeb.UI` primitives, component groups, LiveView pages, CSS/JS hooks, recursive fixture/proof inputs, tests, docs, and one-off patterns unless the omission appears in `documented_exclusions` with `source`, `reason`, and `reviewed_by_phase`.
    Generated/vendor/binary/report-heavy paths are intentionally excluded only by the documented recursive scanner rules; any other fixture/proof source omission is a failing condition.
