...
**Canonical proof chain:** `.planning/phases/47-release-packaging-and-docs-truth/47-VERIFICATION.md` through `.planning/phases/51-default-lane-verifier-hardening-and-support-truth-re-closeout/51-VERIFICATION.md`
**Archive note:** Closed on 2026-05-26 after `.planning/v2.2-MILESTONE-AUDIT.md` returned `passed` with all requirements, phases, integrations, and flows satisfied. The milestone shipped with a publishable package/docs lane, a generated-host Phoenix adoption proof, and one canonical closeout chain: `MIX_ENV=dev mix scoria.release_preview` followed by `MIX_ENV=test mix test.adoption`.
...
- [x] **Phase 49: Support truth and adoption closeout** - Align docs, task output, and verification lanes to the shipped adoption order and close the milestone with bounded proof.
- [x] **Phase 50: Release-preview CI truth and Phase 47 verification** - Restore a truthful CI-safe release-preview lane and close the missing Phase 47 verification gap.
- [x] **Phase 51: Default-lane verifier hardening and support-truth re-closeout** - Make the canonical adoption verifier bounded and green again, then re-close Phase 49 with executable proof.
...
**Plans**: 4 plans
- [x] `48-01-PLAN.md` — Harden and verify the installer contract against fresh-host mutations and idempotency expectations.
- [x] `48-02-PLAN.md` — Build the canonical consumer-app fixture or equivalent host-app harness for dependency, install, migrate, and route proof.
- [x] `48-03-PLAN.md` — Extend the consumer proof through `Scoria.start_run/2`, readback, and operator evidence inspection on the default lane.
- [x] `48-04-PLAN.md` — Add bounded verification coverage proving optional knowledge and semantic surfaces are not hidden prerequisites for the default lane.
...
**Plans**: 3 plans
- [x] `50-01-PLAN.md` — Re-scope the release-preview CI step to the supported env and lock the maintainer command contract.
- [x] `50-02-PLAN.md` — Re-run the bounded package/docs proofs and write `47-VERIFICATION.md`.
- [x] `50-03-PLAN.md` — Repair requirements and roadmap bookkeeping for the now-verified packaging/docs phase.
...
| 48. Host-app install contract and consumer proof | 4/4 | Complete | 2026-05-26 |
...
- Added a real publish-facing docs lane with versioned source links, explicit package inventory, and a bounded `mix scoria.release_preview` contract.
- Hardened `mix scoria.install` around truthful mutation reporting, idempotency, copied-migration boundaries, and explicit optional-lane messaging.
- Proved a fresh Phoenix host can fetch Scoria, install it, migrate, render `/scoria`, start one durable run, and inspect operator evidence without optional knowledge or semantic setup.
- Repaired the maintainer closeout lane and the default-lane adoption verifier so the shipped support story is backed by green executable proof.

### Issues Deferred

- Advanced bounded-handoff example expansion unless real adopter evidence proves the current lane hierarchy is still confusing.
- Package-family decomposition into multiple Hex libraries.
- External semantic cache backends and ANN tuning controls.

### Technical Debt Incurred

- `mix scoria.release_preview` still emits two non-failing docs warnings: `README.md` references `LICENSE`, and `Scoria.Knowledge.Source.t()` is referenced as undefined/private in generated docs.
